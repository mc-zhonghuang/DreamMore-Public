...
import net.ccbluex.liquidbounce.rename.modules.misc.AntiBot
import net.ccbluex.liquidbounce.rename.modules.misc.Teams
import net.ccbluex.liquidbounce.rename.modules.player.Blink
...
@ModuleInfo(name = "KillAura2", description = "Automatically attacks targets around you.",
    category = ModuleCategory.COMBAT, autoDisableMode = Module.AutoDisableMode.WORLDCHANGE)
class KillAura2 : Module() {
    ...
    // Modes
    private val rotations = ListValue("RotationMode", arrayOf("Vanilla", "Other","BackTrack", "LiquidSensePlus","Test"), "BackTrack")
    ...
    private val hiteffect = ListValue("HitEffect", arrayOf("Lightningbolt","Blood","None"),"None")
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        when(hiteffect.get().toLowerCase()){
            ...
            "blood" ->{
                repeat(10) {
                    mc2.effectRenderer.spawnEffectParticle(EnumParticleTypes.BLOCK_CRACK.particleID, target!!.posX, target!!.posY + target!!.height / 2, target!!.posZ,
    ...
    private fun isEnemy(entity: IEntity?): Boolean {
        if (classProvider.isEntityLivingBase(entity) && entity != null && (EntityUtils.targetDead || isAlive(entity.asEntityLivingBase())) && entity != mc.thePlayer) {
            ...
            if (EntityUtils.targetPlayer && classProvider.isEntityPlayer(entity)) {
                ...
                if (player.spectator || AntiBot.isBot(player) || EntityUtils.isUser(player.unwrap()))
                    return false
