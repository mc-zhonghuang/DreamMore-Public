...
class EditorPanel(private val hudDesigner: GuiHudDesigner, var x: Int, var y: Int) : MinecraftInstance() {
    ...
    private fun drawCreate(mouseX: Int, mouseY: Int) {
        ...
        for (element in elements) {
            ...
            Fonts.roboto35.drawString(name, x + 2.0f, y + height.toFloat(), Color.WHITE.rgb)

            val stringWidth = Fonts.roboto35.getStringWidth(name)
        ...
        Fonts.roboto35.drawString("§lCreate element", x + 2F, y + 3.5F, Color.WHITE.rgb)
    }
    ...
    private fun drawSelection(mouseX: Int, mouseY: Int) {
        ...
        Fonts.roboto35.drawString("§lCreate element", x + 2.0f, y.toFloat() + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height
                && mouseY <= y + height + 10)
            create = true
        ...
        Fonts.roboto35.drawString("§lReset", x + 2.toFloat(), y.toFloat() + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height
                && mouseY <= y + height + 10)
        ...
        Fonts.roboto35.drawString("§lAvailable Elements", x + 2.0f, y + height.toFloat(), Color.WHITE.rgb)
        height += 10
        realHeight += 10

        for (element in LiquidBounce.hud.elements) {
            Fonts.roboto35.drawString(element.name, x + 2, y + height, Color.WHITE.rgb)

            val stringWidth = Fonts.roboto35.getStringWidth(element.name)
            if (width < stringWidth + 8)
                width = stringWidth + 8
        ...
        Fonts.roboto35.drawString("§lEditor", x + 2F, y + 3.5f, Color.WHITE.rgb)
    }
    ...
    private fun drawEditor(mouseX: Int, mouseY: Int) {
        ...
        // X
        Fonts.roboto35.drawString("X: ${"%.2f".format(element.renderX)} (${"%.2f".format(element.x)})", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Y
        Fonts.roboto35.drawString("Y: ${"%.2f".format(element.renderY)} (${"%.2f".format(element.y)})", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Scale
        Fonts.roboto35.drawString("Scale: ${"%.2f".format(element.scale)}", x + 2, y + height, Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Horizontal
        Fonts.roboto35.drawString("H:", x + 2, y + height, Color.WHITE.rgb)
        Fonts.roboto35.drawString(element.side.horizontal.sideName,
        ...
        // Vertical
        Fonts.roboto35.drawString("V:", x + 2, y + height, Color.WHITE.rgb)
        Fonts.roboto35.drawString(element.side.vertical.sideName,
                x + 12, y + height, Color.GRAY.rgb)
        ...
        for (value in element.values) {
            when (value) {
                is BoolValue -> {
                    // Title
                    Fonts.roboto35.drawString(value.name, x + 2, y + height, if (value.get()) Color.WHITE.rgb else Color.GRAY.rgb)

                    val stringWidth = Fonts.roboto35.getStringWidth(value.name)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8
                ...
                is FloatValue -> {
                    ...
                    Fonts.roboto35.drawString(text, x + 2, y + height, Color.WHITE.rgb)

                    val stringWidth = Fonts.roboto35.getStringWidth(text)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8
                ...
                is ListValue -> {
                    // Title
                    Fonts.roboto35.drawString(value.name, x + 2, y + height, Color.WHITE.rgb)
                    ...
                    for (s in value.values) {
                        // Value title
                        val text = "§c> §r$s"
                        Fonts.roboto35.drawString(text, x + 2, y + height, if (s == value.get()) Color.WHITE.rgb else Color.GRAY.rgb)
        ...
        // Header
        RenderUtils.drawRect(x, y, x + width, y + 12, ClickGUI.generateColor().rgb)
        Fonts.roboto35.drawString("§l${element.name}", x + 2F, y + 3.5F, Color.WHITE.rgb)

        // Delete button
        if (!element.info.force) {
            val deleteWidth = x + width - Fonts.roboto35.getStringWidth("§lDelete") - 2F
            Fonts.roboto35.drawString("§lDelete", deleteWidth, y + 3.5F, Color.WHITE.rgb)
