...
public class GuiDirectLogin extends WrappedGuiScreen {
    ...
    public void initGui() {
        ...
        username = classProvider.createGuiTextField(2, Fonts.roboto40, getRepresentedScreen().getWidth() / 2 - 100, 60, 200, 20);
        username.setFocused(true);
        username.setMaxStringLength(Integer.MAX_VALUE);
        password = classProvider.createGuiPasswordField(3, Fonts.roboto40, getRepresentedScreen().getWidth() / 2 - 100, 85, 200, 20);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.roboto40.drawCenteredString("Direct Login", getRepresentedScreen().getWidth() / 2.0f, 34, 0xffffff);
        Fonts.roboto35.drawCenteredString(status == null ? "" : status, getRepresentedScreen().getWidth() / 2.0f, getRepresentedScreen().getHeight() / 4.0f + 60, 0xffffff);

        username.drawTextBox();
        password.drawTextBox();

        if (username.getText().isEmpty() && !username.isFocused())
            Fonts.roboto40.drawCenteredString("§7Username / E-Mail", getRepresentedScreen().getWidth() / 2.0f - 55, 66, 0xffffff);

        if (password.getText().isEmpty() && !password.isFocused())
            Fonts.roboto40.drawCenteredString("§7Password", getRepresentedScreen().getWidth() / 2.0f - 74, 91, 0xffffff);
