...
public class GuiAdd extends WrappedGuiScreen {
    ...
    public void initGui() {
        ...
        username = classProvider.createGuiTextField(2, Fonts.roboto40, representedScreen.getWidth() / 2 - 100, 60, 200, 20);
        username.setFocused(true);
        username.setMaxStringLength(Integer.MAX_VALUE);
        password = classProvider.createGuiPasswordField(3, Fonts.roboto40, representedScreen.getWidth() / 2 - 100, 85, 200, 20);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.roboto40.drawCenteredString("Add Account", representedScreen.getWidth() / 2.0f, 34, 0xffffff);
        Fonts.roboto35.drawCenteredString(status == null ? "" : status, representedScreen.getWidth() / 2.0f, representedScreen.getHeight() / 4.0f + 60, 0xffffff);

        username.drawTextBox();
        password.drawTextBox();

        if (username.getText().isEmpty() && !username.isFocused())
            Fonts.roboto40.drawCenteredString("§7Username / E-Mail", representedScreen.getWidth() / 2 - 55, 66, 0xffffff);

        if (password.getText().isEmpty() && !password.isFocused())
            Fonts.roboto40.drawCenteredString("§7Password", representedScreen.getWidth() / 2 - 74, 91, 0xffffff);
