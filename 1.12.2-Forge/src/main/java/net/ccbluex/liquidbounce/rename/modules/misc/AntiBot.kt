...
object AntiBot : Module() {
    private val modeValue = ListValue("Mode", arrayOf("HuaYuTing", "Custom"), "Custom")
    ...
    fun isBot(entity: IEntityLivingBase): Boolean {
        ...
        if (modeValue.equals("HuaYuTing", true)) {
            if (!ground.contains(entity.entityId))
                return true
            if (invalidGround.getOrDefault(entity.entityId, 0) >= 10)
                return true
        } else {
