...
import net.ccbluex.liquidbounce.injection.backend.EntityPlayerImplKt;
...
public final class EntityUtils extends MinecraftInstance {
    ...
    public static boolean isUser(EntityPlayer player) {
        final MoreChecker checker = (MoreChecker) LiquidBounce.moduleManager.getModule(MoreChecker.class);
        if (checker.getState() && checker.getNoHit().get()) {
            return LiquidBounce.getMoreUsers().contains(player.getDisplayName().getFormattedText().replace("§r", "")) || LiquidBounce.getMoreUsers().contains(player.getName());
        }
            ...
            if (targetInvisible || !entity.isInvisible()) {
                if (targetPlayer && classProvider.isEntityPlayer(entity)) {
                    ...
                    if (canAttackCheck) {
                        ...
                        if (isUser(EntityPlayerImplKt.unwrap(entityPlayer)))
                            return false;
