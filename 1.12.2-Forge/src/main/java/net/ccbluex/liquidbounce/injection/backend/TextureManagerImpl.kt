...
class TextureManagerImpl(val wrapped: TextureManager) : ITextureManager {
    ...
    override fun loadTexture(textureLocation: ResourceLocation, textureObj: ITextureObject): Boolean = wrapped.loadTexture(textureLocation.also { unwrap() }, textureObj.also { unwrap() })
    override fun bindTexture(image: ResourceLocation) = wrapped.bindTexture(image.also { unwrap() })
    override fun getTexture(image: ResourceLocation): ITextureObject? = wrapped.getTexture(image.also { unwrap() })
